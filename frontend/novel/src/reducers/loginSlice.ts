...
export const loginSlice = createSlice({
  ...
  // 비동기 액션처리에 대한 상태 관리
  // Promise : fulfilled(성공시) pending(대기) rejected(거부시)
    ...
    builder
      .addCase(loginPostAsync.fulfilled, (state, action) => {
        ...
        if (action.payload.accessToken) {
          setCookie('member', JSON.stringify(action.payload), 1); // 하루
        }
      })
