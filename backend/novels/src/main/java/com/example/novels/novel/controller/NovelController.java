...
public class NovelController {
    ...
    @Operation(summary = "novel 상세조회", description = "novel 상세조회 API")
    @GetMapping("/{id}")

    public NovelDTO getRow(
            @Parameter(description = "novel id 값", example = "1", required = true) @PathVariable("id") Long id) {
    ...
    public Long putRow(@PathVariable("id") Long id, @RequestBody NovelDTO dto) {
    ...
    @Operation(summary = "novel 수정", description = "novel 수정 API - 이용 가능, 장르변경")
    @PreAuthorize("hasAnyRole('MEMBER','ADMIN')")
    @PutMapping("/edit/{id}")
    public Long putNovel(@PathVariable("id") Long id, @RequestBody NovelDTO dto) {
        log.info("novel 수정 요청 {} {}", id, dto);
        dto.setId(id);
